    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.status === "1" && response.data.data) {
                ...
                fetchOrderHistory(response.data.data.gateway_transaction_id);
            } else {
                ...
                <div className="w-full max-w-md bg-white rounded-lg shadow-2xl p-6">
                    <div className="flex border-b mb-6">
                        <button
                            ...
                            className={`flex-1 py-2 text-sm font-semibold transition-colors ${activeTab === "order" ? "border-b-2 border-blue-600 text-blue-600" : "text-gray-500 hover:text-gray-800"}`}
                        >
                            Order Number
                        </button>
                        <button
                            onClick={() => setActiveTab("tracking")}
                            className={`flex-1 py-2 text-sm font-semibold transition-colors ${activeTab === "tracking" ? "border-b-2 border-blue-600 text-blue-600" : "text-gray-500 hover:text-gray-800"}`}
                        >
                            Tracking Number
                        </button>
                    </div>
...
const OrderDetailsDisplay = ({ order, history }) => {
    ...
    const getStatusIcon = (status) => {
        const icons = {
            "Order Placed": <FaClipboardList />, "Processing": <FaBox />,
            "Shipped": <FaShippingFast />, "Delivered": <FaCheckCircle />,
        };
                        ...
                        return (
                            <React.Fragment key={step}>
                                <div className="flex flex-col items-center">
                                    ...
                                    <p className={`text-xs text-center mt-2 font-semibold ${isCompleted ? "text-blue-600" : "text-gray-500"}`}>{step}</p>
                                    {historyItem && (<p className="text-xs text-gray-400 mt-1">{new Date(historyItem.created_at).toLocaleDateString()}</p>)}
                                </div>
                                {index < timelineSteps.length - 1 && (<div className={`flex-1 h-1 mt-6 ${isCompleted ? "bg-blue-600" : "bg-gray-200"}`}></div>)}
                            </React.Fragment>
                        );
                    })}
                </div>

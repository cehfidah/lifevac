...
export default function OrderDetails() {
    ...
    if (!transaction) return <div className="p-4 text-red-500">Transaction not found</div>;
        ...
        <div className="p-4 space-y-6 max-w-4xl mx-auto">
            ...
            <div className="bg-white rounded-xl shadow-md p-4 space-y-2">
                ...
                <div className="flex flex-wrap gap-4 text-sm text-gray-700">
                    <p>Status:
                        <span
                            className={`ml-2 px-2 py-1 rounded-full text-white ${transaction.payment_status === "COMPLETED"
                                ? "bg-green-500"
                                : "bg-yellow-500"
                                }`}
                        >
            ...
            <div className="bg-white rounded-xl shadow-md p-4">
                ...
                <div className="text-sm text-gray-700 space-y-1">
                    <p>{transaction.shipping_address.firstName} {transaction.shipping_address.lastName}</p>
                    <p>{transaction.shipping_address.address}, {transaction.shipping_address.apt}</p>
                    <p>{transaction.shipping_address.city}, {transaction.shipping_address.state} - {transaction.shipping_address.zip}</p>
                    <p>{transaction.shipping_address.country}</p>
                    <p>Phone: +{transaction.shipping_address.phoneCode} {transaction.shipping_address.phone}</p>
                </div>
            </div>
                ...
                {transaction.product_detail.map((item, idx) => (
                    <div key={idx} className="flex gap-4 border-b pb-4">
                        ...
                        <div className="flex flex-col text-sm text-gray-700 space-y-1">
                            <h4 className="font-semibold text-gray-900">{item.sectionTitle}</h4>
                            <p>{item.title} - {item.kits}</p>
